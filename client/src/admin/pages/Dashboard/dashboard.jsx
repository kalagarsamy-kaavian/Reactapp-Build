...
export default function Empcomplete() {
    ...
    useEffect(()=>{
    fetch('http://localhost:3002/tlcomplete').then(res=>res.json()).then(data=>setCount(data));
    fetch('http://localhost:3002/tlongoing').then(res=>res.json()).then(data=>setNum(data));
    fetch('http://localhost:3002/tlcount').then(res=>res.json()).then(data=>setTeam(data));
})
