...
app.patch('/remcomplete/:Teamname', async(req) => {
    const { Teamname} = req.params;
    console.log('remcomplete',Teamname)
    //console.log(id);
    const data=await newModel.find({Teamname:Teamname});
    console.log(data);
    let[first,second,third,fourth,fifth]=data;
//     await newEmployee.updateMany(
//         { Empid: { $in: [first.Empid, second.Empid, third.Empid, fourth.Empid, fifth.Empid] } },
//         { $set: { Projectstatus : "COMPLETED" } },
//         {multi: true}
//      )
//    await newModel.updateMany({Teamname:Teamname}, { $set: { Projectstatus: "COMPLETED" }}).then(updateStatus => {
//        res.json(updateStatus);
//    console.log('Btn updated')
//    })

  // await newEmployee.updateMany({Teamname:Teamname}, { $set: { Projectstatus: "COMPLETED" }})
})
...
app.post('/olist/:id',async(req,res)=>{
    const {id}=req.params;
     console.log(id)
   await  newModel.find({$and:[{Empid:id},{Projectstatus:'Ongoing'}]}).then(data=>{
        let[first,...rest]=data;
